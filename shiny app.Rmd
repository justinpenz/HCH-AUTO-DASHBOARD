      ...
      tabPanel('BUILD',
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            start = Sys.Date() - 2, end = Sys.Date() + 2, separator = 'to')),
          mainPanel(dataTableOutput("table10"),
...
########################MAIN DATASET FROM UPLOAD############################    
  ...
  observeEvent(input$pushgrp, {              #IF MAKE GROUP BUTTON IS PUSHED, ADD CURRENT SELECTED DRUGS AS grouped item to values 
    ...
    values$df_data <- bind_rows(values$df_data, data.frame(drug = as.vector(drugs), push_no = as.vector(pushID), grouping = as.vector(grp_name)))
    
    
  })
  
  observeEvent(input$pushsingle, {
    pushID = input$pushsingle
    drugs <- input$selected_drugs_list 
    
    values$df_data <- bind_rows(values$df_data, data.frame(drug = as.vector(drugs), push_no = as.vector(pushID), grouping = as.vector(drugs)))

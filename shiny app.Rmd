        ...
        daterange0 <- as.list(unlist(strsplit(as.character(input$dateRange0), 'to')))
          ...
          working_df <- df %>% 
            ...
            totals_by_Timeframe1 <- working_df %>% 
              ...
              group_by(grouping) %>% summarize(Timeframe1 = sum(Total.Spend, na.rm = T))
            
            totals_by_Timeframe2 <- working_df %>% 
              filter(ymd(Date) >= ymd(daterange2$min) & ymd(Date) < ymd(daterange2$max))%>%
              group_by(grouping) %>% summarize(Timeframe2 = sum(Total.Spend, na.rm = T))
            
             table <- full_join(totals_by_month, totals_by_Timeframe1, by = "grouping") %>% 
              full_join(totals_by_Timeframe2, by ='grouping') %>% 
